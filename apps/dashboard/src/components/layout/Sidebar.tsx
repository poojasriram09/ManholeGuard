...
const navItems = [
  ...
  { to: '/manholes', label: 'Manholes', icon: (
  ...
  { to: '/heatmap', label: 'Heatmap', icon: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l5.447 2.724A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
  )},
...
export default function Sidebar() {
  ...
  return (
    <aside className={`fixed left-0 top-0 h-full glass text-white transition-all duration-300 z-30 flex flex-col ${sidebarOpen ? 'w-64' : 'w-16'}`}>
      <div className="p-4 border-b border-border">
        <div className={`flex items-center gap-2.5 ${sidebarOpen ? '' : 'justify-center'}`}>
          <svg className="w-7 h-7 text-accent flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <nav className="mt-2 space-y-0.5 flex-1 overflow-y-auto px-2">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              `relative flex items-center rounded-lg px-3 py-2.5 text-sm transition-all duration-200 ${
                isActive
                  ? 'bg-accent-muted text-accent-strong shadow-sm'
      ...
      <div className="border-t border-border p-3">
        <button onClick={logout}
          className={`flex items-center gap-2.5 text-text-muted hover:text-danger text-sm transition-colors w-full rounded-lg px-3 py-2 hover:bg-danger-muted ${sidebarOpen ? '' : 'justify-center'}`}>
