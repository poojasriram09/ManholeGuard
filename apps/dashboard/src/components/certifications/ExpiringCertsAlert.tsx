...
export default function ExpiringCertsAlert({ count, onClick }: ExpiringCertsAlertProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-yellow-50 border border-yellow-300 rounded-lg px-4 py-3 flex items-center gap-3 ${onClick ? 'cursor-pointer hover:bg-yellow-100' : ''} transition-colors`}
    >
